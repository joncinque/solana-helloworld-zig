...
fn processInstruction(program_id: *sol.public_key.PublicKey, accounts: []sol.account.Account, data: []const u8) ProgramError!void {
    sol.log.print("Hello zig program {f}", .{program_id});
    for (accounts, 0..) |account, i| {
        sol.log.print("Hello Account {d}: {f}", .{ i, account.info().owner_id });
    }
    sol.log.print("Hello Data: {any}", .{data});
    ...
    switch (instruction_type.*) {
        ix.InstructionType.init => {
            ...
            sol.log.print("Hello init {d}", .{init.account_type});
        ...
        ix.InstructionType.add => {
            ...
            sol.log.print("Hello add {d}", .{add.amount});
            const account = accounts[0];
            const account_type: state.AccountType = @enumFromInt(account.data()[0]);
            switch (account_type) {
                state.AccountType.Uninitialized => return ProgramError.Uninitialized,
